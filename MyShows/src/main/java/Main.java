...
//        for (int i = 1; i <= 1; i++) {
        for (int i = 0; i <= 334; i++) {
            tempArray.clear();
            doc = Jsoup.connect("https://myshows.me/search/all/?page=" + i).timeout(20000).userAgent("Yandex").get();
            ...
            for (Element el : currentpagedata) {
                ...
                String name = findValue(el.toString(),
                        ...
                        '<');
                Series series = new Series(name, "https://myshows.me/view" + href);
...
//        loadData(seriesArray);    // Uncommit for load information
